...
- (CAShapeLayer *)if_layer {
    ...
    if (!layer) {
        ...
        self.tmo_layer = layer;
    }
    return layer;
}

- (void)setTmo_layer:(CAShapeLayer *)tmo_layer {
    objc_setAssociatedObject(self, &if_layer_key, tmo_layer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setIF_corner:(IFCorner *)tmo_corner {
    objc_setAssociatedObject(self, &if_corner_key, tmo_corner, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)if_updateCornerRadius:(void (^)(IFCorner *))handler {
    if (handler) {
        handler(self.tmo_corner);
    }
    CGColorRef fill = self.tmo_corner.fillColor.CGColor;
    if (!fill || CGColorEqualToColor(fill, [UIColor clearColor].CGColor)) {
        if (!self.backgroundColor || CGColorEqualToColor(self.backgroundColor, [UIColor clearColor].CGColor)) {
            if (!self.tmo_corner.borderColor || CGColorEqualToColor(self.tmo_corner.borderColor.CGColor, [UIColor clearColor].CGColor)) {
                return;
            }
        }
    ...
    if (self.tmo_corner.borderWidth < 0) {
        self.tmo_corner.borderWidth = 0;
    }
    
    CGRect tmo_frame = self.bounds;
    tmo_frame.origin.x = self.tmo_corner.borderWidth * 0.5;
    tmo_frame.origin.y = self.tmo_corner.borderWidth * 0.5;
    tmo_frame.size.width -= self.tmo_corner.borderWidth;
    tmo_frame.size.height -= self.tmo_corner.borderWidth;
    self.if_layer.frame = tmo_frame;
    
    UIBezierPath *path = [UIBezierPath bezierPath];
    CGFloat height = tmo_frame.size.height;
    CGFloat width = tmo_frame.size.width;
    //左下
    ...
    self.if_layer.strokeColor = self.tmo_corner.borderColor.CGColor;
    self.if_layer.lineWidth = self.tmo_corner.borderWidth;
